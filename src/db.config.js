...
async function connectdb() {
  try {
    ...
    if (!maxInvNum) await SettingModel.create({ key: 'maxInvNum', value: 999999 })
    if (maxInvNum) {
      const maxInvNum = await SettingModel.findOne({ where: { key: 'maxInvNum' } })
      if (maxInvNum != 999999) SettingModel.update({ value: 999999 }, { where: { key: 'maxInvNum' } })
    }
    ...
    const tranNo = await SettingModel.findOne({ where: { key: 'tranNo' } })
    if (!tranNo) await SettingModel.create({ key: 'tranNo', value: 'SR' })
