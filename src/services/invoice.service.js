...
module.exports = function () {
  ipcMain.handle('getInvoice', async (_, obj = {}) => {
    ...
    const orders = await sequelize.query(`
      SELECT o.id, o.qty, o.price, p.dividend, o.isPrinted, c.id customerId, c.name customer, o.address, c.phone,
  ...
  ipcMain.handle('getCustomerInvoice', async (_, orderNos) => {
    const orders = await sequelize.query(`
      ...
      p.name product, p.id productId, o.qty, o.price, c.phone, o.code, p.dividend, o.invNumber
      FROM tb_order o
      LEFT JOIN tb_customer c ON o.customerId = c.id
      LEFT JOIN tb_product p ON o.productId = p.id
